...
class Chapter:
    ...
    def find_mapping(self, response, *mappings):
        ...
        # specific handling for dialogs
        if self.game_manager.dialog.detect_dialog(text):
            return self.game_manager.dialog.find_mapping(text, tags)

        # remove subject, possessive, pronouns...
        response = self._skin_response(text, tags)
